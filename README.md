...
| CPU fallback | false |
| 重み勾配dW MatMulのHTP実行 | SUCCESS。loss/dP/optimizerはCPU側 |
| backward全体/optimizerのHTP実行 | 未実装 |
...
QNN_HTP_FULL_STEP
QNN_HTP_DW_CHECK
QNN_HTP_FORWARD_HTP_DW_TRAINING
QNN_HTP_FORWARD_HTP_DW_BENCHMARK
```

`QNN_HTP_DW_CHECK`、`QNN_HTP_FORWARD_HTP_DW_TRAINING`、`QNN_HTP_FORWARD_HTP_DW_BENCHMARK`では、forwardと、backwardの一部である重み勾配`dW = X^T dP`をHTPで実行します。loss、`dP`、SGD optimizerはCPUです。詳細は[docs/qnn-htp-backward-dw.md](docs/qnn-htp-backward-dw.md)を参照してください。
...
| `QNN_HTP_DW_CHECK` | HTP forward、重み勾配`dW` | loss、`dP`、SGD | SUCCESS、小shape CPU/HTP dW一致 |
| `QNN_HTP_FORWARD_HTP_DW_TRAINING` | HTP forward、重み勾配`dW` | loss、`dP`、SGD | SUCCESS、5 seed×640 step |
| `QNN_HTP_FORWARD_HTP_DW_BENCHMARK` | HTP forward、重み勾配`dW` | loss、`dP`、SGD | SUCCESS、CPU dW参照を測定経路から除外 |
